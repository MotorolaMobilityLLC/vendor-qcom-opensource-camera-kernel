...
static int dw9784_check_fw_download(struct camera_io_master * io_master_info, const uint8_t *fwData, uint32_t fwSize)
{
	uint8_t ret;
...
static int dw9784_check_if_download(struct camera_io_master * io_master_info)
{
	uint16_t FMC;
...
static void dw9784_post_firmware_download(struct camera_io_master * io_master_info)
{
	uint16_t fwchecksum = 0;
...
int32_t dw9784_fw_update(struct cam_ois_ctrl_t *o_ctrl, const struct firmware *fw)
{
	...
	int fw_type           = CAMERA_SENSOR_I2C_TYPE_WORD;
	int prog              = 0x8000;

	if (!dw9784_check_fw_download(&(o_ctrl->io_master_info), fw->data, fw->size)) {
		CAM_INFO(CAM_OIS, "dw9784 Skip firmware download.");
		return 0;
	}

	CAM_INFO(CAM_OIS, "dw9784 Firmware download started.");
	...
	if (!vaddr) {
		CAM_ERR(CAM_OIS, "dw9784 Failed in allocating i2c_array");
		return -ENOMEM;
	}

	CAM_INFO(CAM_OIS, "dw9784 FW prog size:%d.", total_bytes);
	CAM_INFO(CAM_OIS, "dw9784 fw len: %d, addr_type: %d, data_type: %d, chunck: %d, fw_type:%d",
				total_bytes,
	...
	for (total_idx = 0, ptr = (uint8_t *)fw->data; total_idx < total_bytes;) {
		for (packet_idx = 0;
			(packet_idx < (txn_data_size/fw_type)) && (total_idx + (packet_idx*fw_type) < total_bytes);
			packet_idx ++, ptr += fw_type)
		{
			int regAddrOffset = total_idx/fw_type + packet_idx;

			if (total_idx >= (total_bytes - DW9784_IF_SIZE)) {
				i2c_reg_setting.reg_setting[packet_idx].reg_addr =
					prog + regAddrOffset - (total_bytes - DW9784_IF_SIZE)/2;
				...
				i2c_reg_setting.reg_setting[packet_idx].reg_addr = prog + regAddrOffset;
				i2c_reg_setting.reg_setting[packet_idx].reg_data = (uint32_t)(*ptr << 8) | *(ptr+1);
				i2c_reg_setting.reg_setting[packet_idx].delay = 0;
				i2c_reg_setting.reg_setting[packet_idx].data_mask = 0;
		...
		rc = camera_io_dev_write_continuous(&(o_ctrl->io_master_info), &i2c_reg_setting, CAM_SENSOR_I2C_WRITE_SEQ);
		if (rc < 0) {
			CAM_ERR(CAM_OIS, "OIS FW(prog) size(%d) download failed. %d", total_bytes, rc);
			goto release_firmware;
		}

		total_idx += (packet_idx*fw_type);
