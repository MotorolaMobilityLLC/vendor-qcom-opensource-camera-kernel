	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (rc == 0) {
		...
		CAM_DBG(CAM_SENSOR,
			"Sensor[%d] Received Res Info: num_batched_frames: %d ",
	...
	struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_DBG(CAM_SENSOR,
		"Sensor Addr: 0x%x sensor_id: 0x%x sensor_mask: 0x%x sensor_pipeline_delay:0x%x probe_sensor_slave_addr:0x%x i2c_addr_switch: 0x%x second_i2c_address: 0x%x i2c_switch_reg_addr: 0x%x",
		s_ctrl->sensordata->slave_info.sensor_id_reg_addr,
		s_ctrl->sensordata->slave_info.sensor_id,
		s_ctrl->sensordata->slave_info.sensor_id_mask,
		s_ctrl->pipeline_delay,
