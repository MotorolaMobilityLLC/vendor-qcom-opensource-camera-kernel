...
typedef enum {
	REGULATOR_IOVDD,
#ifdef CONFIG_VANTG_DTB
	REGULATOR_LDO_CUSTOM,
#endif
	REGULATOR_VCMVDD,
	REGULATOR_OISVDD,
	REGULATOR_NUM,
...
/*----------------------For VANTG SEM1218S------------------------------*/
static struct cam_sensor_i2c_reg_array mot_ois_init_sem1218s[] = {
	{0x0002, 0x00, 1},
};
...
struct cam_sensor_i2c_reg_array fw_cali_reg_array[CHUNCK_SIZE] = {0};
static int32_t mot_ois_bu63169_fw_download(struct camera_io_master * io_master_info, const struct firmware *prog, uint16_t op_reg)
{
	...
	while (remain_bytes) {
		pCaliRegArray = fw_cali_reg_array;
		...
		cali_setting.reg_setting = fw_cali_reg_array;
